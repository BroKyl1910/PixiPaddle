class Ball {
  constructor(gameAreaBounds, renderer, setScene) {
    ...
    this.pos = {
      x:
        Math.random() * (gameAreaBounds.topRight.x - gameAreaBounds.topLeft.x) +
        gameAreaBounds.topLeft.x,
      y:
        Math.random() *
          (gameAreaBounds.bottomRight.y - gameAreaBounds.topRight.y) +
        gameAreaBounds.topRight.y,
    }
    this.dir = {
      x: 1,
      y: 1,
  ...
  #checkWallCollisions() {
    if (this.pos.y <= this.gameAreaBounds.topLeft.y) {
      ...
      this.pos.y >=
      this.gameAreaBounds.bottomRight.y - this.radius * 2
    ) {
      this.dir.y = -1
      this.speed += this.speedIncrement
    }
    ...
    if (this.pos.x <= this.gameAreaBounds.topLeft.x) {
      ...
      this.pos.x >=
      this.gameAreaBounds.bottomRight.x - this.radius * 2
    ) {
      this.dir.x = -1
      this.speed += this.speedIncrement
    }
  ...
  #ballCollidingWithPaddle(paddleBounds) {
    if (
      this.pos.x + this.radius * 2 >= paddleBounds.topLeft.x &&
      this.pos.x <= paddleBounds.topRight.x && // x positions are colliding
      this.pos.y >= paddleBounds.topLeft.y &&
      this.pos.y + this.radius * 2 <= paddleBounds.bottomLeft.y
    )
      // y positions are colliding
      return true
